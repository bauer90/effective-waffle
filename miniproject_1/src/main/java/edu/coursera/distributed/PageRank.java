...
//            Tuple2<Website, Double> value = kv._2();
            ...
            Iterator<Integer> edgeIterator = ws.edgeIterator();
            while (edgeIterator.hasNext()) {
                Integer wsID = edgeIterator.next();
                contribs.add(new Tuple2(wsID, currentRank / ws.getNEdges()));
            }
            return contribs;
        });

        return newRanks.reduceByKey((Double x, Double y) -> x + y).mapValues((x) -> x * 0.85 + 0.15);
    }
}
